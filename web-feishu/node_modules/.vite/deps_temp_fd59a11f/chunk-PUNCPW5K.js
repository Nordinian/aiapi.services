...
var Lang = {
  /* API */
  clamp: (number8, lower2, upper) => {
    if (lower2 > upper)
      return Math.min(lower2, Math.max(upper, number8));
    return Math.min(upper, Math.max(lower2, number8));
  },
  round: (number8) => {
...
function cleanArray(array7) {
  for (let index5 = 0; index5 < array7.length; index5++) {
    const isPropertyExist = objectHasOwnProperty(array7, index5);
    if (!isPropertyExist) {
      array7[index5] = null;
    }
  }
...
var renderKatex = __name(async (text2, config2) => {
  ...
  if (true) {
    const { default: katex } = await import("./katex-V7FQ2FF3.js");
...
function shuffler(random) {
  return function shuffle(array7, i0 = 0, i1 = array7.length) {
    let m = i1 - (i0 = +i0);
    while (m) {
...
function ticks2(start2, stop, count5) {
  ...
  const reverse5 = stop < start2, [i1, i2, inc] = reverse5 ? tickSpec(stop, start2, count5) : tickSpec(start2, stop, count5);
  if (!(i2 >= i1)) return [];
...
function tickStep2(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  const reverse5 = stop < start2, inc = reverse5 ? tickIncrement2(stop, start2, count5) : tickIncrement2(start2, stop, count5);
...
function tickFormat(start2, stop, count5, specifier) {
  var step = tickStep2(start2, stop, count5), precision;
...
function linearish(scale2) {
  ...
  scale2.nice = function(count5) {
    ...
    while (maxIter-- > 0) {
      step = tickIncrement2(start2, stop, count5);
      if (step === prestep) {
        d[i0] = start2;
        d[i1] = stop;
        return domain(d);
...
function tickStep3(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  const reverse5 = stop < start2, inc = reverse5 ? tickIncrement3(stop, start2, count5) : tickIncrement3(start2, stop, count5);
  return (reverse5 ? -1 : 1) * (inc < 0 ? 1 / -inc : inc);
}
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function ticks5(start2, stop, count5) {
  ...
  function tickInterval(start2, stop, count5) {
    ...
    if (i === tickIntervals.length) return year.every(tickStep3(start2 / durationYear, stop / durationYear, count5));
...
function pie_default() {
  var value = identity_default3, sortValues = descending_default, sort5 = null, startAngle = constant_default3(0), endAngle = constant_default3(tau), padAngle = constant_default3(0);
  function pie(data) {
    var i, n = (data = array_default2(data)).length, j, k2, sum7 = 0, index5 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
*/
//# sourceMappingURL=chunk-PUNCPW5K.js.map
